...
import { camelCaseToUpperCase } from '../../lib/format';
import { useUid } from '../../lib/useUid';
import { Select } from './Select';
// import { format } from '~/lib/format';

type InputType = 'integer' | 'string' | 'text' | 'boolean' | 'float' | 'email' | 'password' | 'hidden' | 'enum' | 'submit';
...
interface RestInputProps extends BaseInputProps {
  type: 'integer' | 'string' | 'text' | 'boolean' | 'float' | 'email' | 'password';
  prop: string;
  label?: string;
  initialValue?: any;
  hint?: string;
...
type InputProps = RestInputProps | HiddenInputProps | EnumInputProps | SubmitInputProps;
type NonSubmitInputProps = RestInputProps | HiddenInputProps | EnumInputProps;

type NonHiddenInputProps = RestInputProps | SubmitInputProps | EnumInputProps;
...
export const Input: React.FC<InputProps> = function Input(props) {
  ...
  switch (props.type) {
    ...
    case 'boolean':
      return (
        <input
          ...
          name={props.prop}
          onChange={(ev) => onChange(props.type === 'boolean' ? ev.target.checked : ev.target.value)}
    ...
    default:
      // @ts-expect-error should not happen in TS
    ...
    <form className={className || 'grid grid-cols-2 space-y-2 relative'} onSubmit={onFormSubmit}>
      {(inputs.filter((i) => i.props.type !== 'hidden') as NonHiddenInputElement[]).map(({ props }) =>
        props.type === 'submit'
          ...
          : [
              <label
                key={props.prop + '-label'}
                className="pr-4 flex flex-row-reverse items-center h-12 first:mt-2"
                htmlFor={uid + props.prop}
              >
                <span>{props.label || (props.prop ? camelCaseToUpperCase(props.prop) : null)}</span>
              </label>,
              <Input
                key={props.prop + '-input'}
                {...props}
                _value={state[props.prop]}
                _onChange={onChangeFactory(props.prop)}
                _formUid={uid}
              />,
            ]
      )}
