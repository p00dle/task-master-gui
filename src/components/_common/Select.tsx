...
import { useHideOnClickOutside } from '../../lib/useHideOnClickOutside';
...
export function SelectItem<T>({ item, onSelect }: SelectItemProps<T>): JSX.Element {
  ...
  const arrowClass = hasSubMenu ? (collapsed ? 'arrow-up' : 'arrow-down') : '';

  return (
    <li className="">
      <div className={'secondary-hover p-1' + arrowClass} onClick={onClick}>
        {label}
      </div>
      {Array.isArray(subMenu) && !collapsed ? (
        <ul className="">
          {subMenu.map((subItem) => (
            <SelectItem key={subItem.label} item={subItem} onSelect={onSelect} />
          ))}
        </ul>
      ) : null}
    </li>
...
export function Select<T>({ value, onChange, pool, disabled = false, name, className = '', id }: SelectProps<T>): JSX.Element {
  ...
  const displayLabel = String(
    items.some((a) => a.value !== undefined) ? (items.find((a) => a.value === value) || { label: '' }).label : value
  );
  return (
    <div className={shouldShow ? 'arrow-up' : 'arrow-down'} ref={ref}>
      <input
        ...
        onClick={disabled ? undefined : shouldShow ? hide : show}
        value={displayLabel}
        disabled={disabled}
        name={name}
        id={id}
      ...
      <ul
        className={
          'py-1 absolute w-full secondary border-2 rounded-md z-[4] select-none max-h-72 overflow-x-auto ' +
          (shouldShow ? 'block' : 'hidden')
        }
      >
        {items.map((item) => (
          <SelectItem item={item} key={item.label} onSelect={onSelect} />
        ))}
      </ul>
    </div>
  );
